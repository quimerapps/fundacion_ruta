...
function Epoch(name,mode,targetelement,multiselect) {
	...
	*/
	function calConfig() {
        ...
        self.minDate = new Date(NowYear-100,1,1);
		self.maxDate = new Date(NowYear+10,12,31);
		...
		if(self.date.canSelect) {
			if(owner.selectMultiple == true) { //if we can select multiple cells simultaneously, add the currently selected self's date to the dates array
				...
				if(owner.mode == 'popup') { //update the target element's value and hide the calendar if in popup mode
					...
					//document.getElementById('edad').value = 'nueva';
...
function calcularEdad()
{
    ...
    if(validate_fecha(fecha)==true)
    {
        ...
        if(edad<0){
        	 ...
        	 alert("Fecha nacimiento incorrecta debe ser menor o igual a hoy");
        	
        }else{
        document.getElementById("edad").value= edad+" años, "+meses+" meses y "+dias+" días";
        }
    }else{
