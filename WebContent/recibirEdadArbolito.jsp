...
<%
	...
	String directorio_ruta = application.getRealPath("imagenes")+"/edades/";

	String id = request.getParameter("id");
	
	String descripcion = request.getParameter("descripcion");
	String edad = request.getParameter("edad");
	String tipo_edad = request.getParameter("tipo_edad");
	
	if(tipo_edad!=null && tipo_edad.equals("A")){
		 edad = ""+(Integer.parseInt(request.getParameter("edad"))*12);
	}
	
	//String us = request.getParameter("us");

	int guardo = 0;
	// Si se ha enviado correctamente el formulario con la imagenes se carga la imagen al servidor	
	int l = 0;
	int campo = 1;
	...
	//i=nombres.length;
								...
								if (i == nombres.length - 1) {
									if (sizeInBytes <= tamano_maximo_archivo) {
										...
										int ultima = bAdministrarPublicaciones.ingresarEdadArbolito(id,descripcion,edad);
										
										File uploadedFile = new File(directorio_ruta + "arbolito_"+ultima+".jpg");
										item2.write(uploadedFile);
										guardo = 1;   


%>
<script>alert('Edad-Imagen registradoa con éxito, se cerrará la ventana. Si esto no sucede haga clic en el botón cerrar'); 
try{
...
%><script language="javascript">window.alert("La foto excede el tamaño máximo de: "+ <%=tamano_maximo_archivo%> +"KB"); window.close();</script>
<%
	}
...
%><script language="javascript">window.alert("Solo se pueden cargar Imágenes"); window.close();</script>
...
<p />
<p />
<form name="form1" action="crearImagenArbolito.jsp?id=<%=id %>">
<%
	if (guardo == 1) {
%>
...
<input class="searchbutton" name="btnVolver" id="btnVolver"
	...
	onclick="document.form1.action='crearImagenArbolito.jsp'; document.form1.submit(); return false;" /></center>
<%
	}
%>
</form>
